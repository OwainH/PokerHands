public class PokerHands {
    ...
    public boolean hasStraight(Hand hand){
        ...
        for(int i = 0; i < HANDSIZE - 1; i++){
            if(hand.getCardByIndex(HANDSIZE -1).getNumber() == 14 && i == HANDSIZE - 2){
                if(hand.getCardByIndex(0).getNumber() == 2){
                    return true;
                }
            }
            else if(hand.getCardByIndex(HANDSIZE - 1).getNumber() == 14 && i == HANDSIZE - 2){

            }
            else if(!(hand.getCardByIndex(i).getNumber() + 1 == hand.getCardByIndex(i + 1).getNumber())){
                return false;
            }
        }
        return true;
